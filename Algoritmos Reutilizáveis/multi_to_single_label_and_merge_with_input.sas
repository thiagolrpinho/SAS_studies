...
data WORK.SINGLELABELS(keep= id_peca area);
	set WORK.labels_id_corrigido;
	array vars{*} area1 area2 area3 area4 area5 area6 area7;
	do i = 1 to dim(vars);
		area = strip(upcase(vars[i]));
		if area ne '' then
			output;
	end;
run;
...
PROC SQL;
  CREATE TABLE WORK.PECAS_COM_LABELS AS 
  SELECT t2.id_peca,
      t2.texto_peca,
